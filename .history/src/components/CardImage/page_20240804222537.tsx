      ...
      {mock.map((item, index) => (
        <div
          ...
          className="group group/item cursor-pointer flex-1 hover:flex-[2] overflow-hidden duration-300 relative transition-shadow hover:shadow-xl hover:shadow-black/30">
            ...
            <Image src={item.image} alt="image" className="w-full h-full object-cover group-hover:scale-110 duration-500 transition-all" />
            <div className="absolute inset-0 z-10 bg-gradient-to-b from-transparent via-transparent to-black group-hover:from-black/70 group-hover:via-black/60 group-hover:to-/70"></div>
        </div>
      ))}
    </div>
  );

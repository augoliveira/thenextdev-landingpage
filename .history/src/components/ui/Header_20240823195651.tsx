    ...
    <>
      ...
      >
        <div className="w-full h-[90px]">
          <header className="max-w-screen-2xl mx-auto text-white flex items-center justify-between h-full">
              ...
              <div className="hidden lgl:flex items-center gap-6 xl:gap-12 uppercase font-titleFont tracking-[3px] text-[14px] font-semibold">
                {navigation?.map((item) => (
                  <Link
                    key={item?.title}
                    href={item?.link}
                    className="relative px-3 group hover:cursor-pointer overflow-h py-1"
                  >
                    {item?.title}
                    <span
